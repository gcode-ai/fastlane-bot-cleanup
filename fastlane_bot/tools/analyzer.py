...
class _DCBase:
    """base class for all staticdata classes, adding some useful methods"""
...
class CPCAnalyzer(_DCBase):
    ...
    def pair_analysis(self, pair, **params):
        ...
        :returns:           an attributed dictionary with the following fields:
            ...
            :curvedf:       curve staticdata as dataframe, as returned by PairData.curve_data
            ...
            :tib, tiq:      trade instruction staticdata frames (target = base / quote token respecitvely)
            ...
            :tib/q_xnoc:    trade instruction staticdata frames for the extended pairs (non-carbon curves only)
        ...
        d["xpairs"] = self.CC.filter_pairs(
            bothin=f"{d.tknb}, {d.tknq}, {CPCContainer.TRIANGTOKENS}"
        )
    ...
    def pool_arbitrage_statistics(
        self, result=None, *, sort_price=True, only_pairs_with_carbon=True
    ):
        """
        ...
        :returns:                   the statistics staticdata in the requested format
        """
        # select all curves that have at least one Carbon pair...
        if only_pairs_with_carbon:
            curves_by_carbon_pair = {
                pair: self.CC.bypairs([pair]) for pair in self.pairsc()
            }
        else:
            curves_by_carbon_pair = {
                pair: self.CC.bypairs([pair]) for pair in self.pairs()
            }
        ...
        pricedf0 = pd.DataFrame(
            prices_l,
            columns="pair,pairf,price,cid,cid0,exchange,vl,itm,b,s,bsv".split(","),
        )
        if sort_price:
            pricedf = pricedf0.drop(["cid", "pairf"], axis=1).sort_values(
                by=["pair", "price", "exchange", "cid0"]
            )
        else:
            pricedf = pricedf0.drop(["cid", "pairf"], axis=1).sort_values(
                by=["pair", "exchange", "cid0"]
            )
    ...
    def price_ranges(self, result=None, *, short=True):
        ...
        df = pd.DataFrame(
            price_l, columns="pair,cid,exch,b,s,p_min,p_max,p_marg".split(",")
        )
