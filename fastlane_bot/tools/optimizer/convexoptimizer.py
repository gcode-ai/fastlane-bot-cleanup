...
class ConvexOptimizer(CPCArbOptimizer):
    ...
    class NofeesOptimizerResult(ConvexOptimizerResult):
        ...
        def trade_instructions(self, ti_format=None):
            ...
            assert (
                ti_format != CPCArbOptimizer.TIF_DFAGGR
            ), "TIF_DFAGGR not implemented for convex optimization"
            assert (
                ti_format != CPCArbOptimizer.TIF_DFPG
            ), "TIF_DFPG not implemented for convex optimization"
            return CPCArbOptimizer.TradeInstruction.to_format(
                result, ti_format=ti_format
            )
