...
from .etl.models.manager import Manager
...
from .utils import log_format, num_format, num_format_float
...
class CarbonBotBase:
    ...
    def __post_init__(self):
        ...
        if self.TxHelpersClass is None:
            self.TxHelpersClass = TxHelpers(ConfigObj=self.mgr)
        assert issubclass(
            self.TxHelpersClass.__class__, TxHelpersBase
        ), f"TxHelpersClass not derived from TxHelpersBase {self.TxHelpersClass}"

        self.db = QueryInterface()
        self.RUN_FLASHLOAN_TOKENS = [*self.mgr.CHAIN_FLASHLOAN_TOKENS.values()]

    @property
    def C(self) -> Any:
        """
        Convenience method self.mgr
        """
        return self.mgr
...
class CarbonBot(CarbonBotBase):
    ...
    def _get_deadline(self, block_number) -> int:
        ...
        """
        block_number = (
            self.mgr.w3.eth.block_number if block_number is None else block_number
        )
        return (
            self.mgr.w3.eth.get_block(block_number).timestamp
            + self.mgr.DEFAULT_BLOCKTIME_DEVIATION
        )
    ...
    def _run(
        ...
        *,
        ...
        if r is None or len(r) == 0:
            self.mgr.logger.info("[bot._run] No eligible arb opportunities.")
            return None

        self.mgr.logger.info(f"[bot._run] Found {len(r)} eligible arb opportunities.")
    ...
    def validate_pool_data(self, arb_opp):
        ...
        bool
        """
        self.mgr.logger.info("[bot.validate_pool_data] Validating pool data...")
        (
            best_profit,
            best_trade_instructions_df,
            best_trade_instructions_dic,
            ...
            if current_pool.exchange_name == "carbon_v1":
                ...
                for idx, balance in enumerate(current_pool.token_balances):
                    if balance != fetched_pool[f"tkn{idx}_balance"]:
                        ...
                        return False
            elif current_pool.exchange_name in self.mgr.UNI_V3_FORKS:
                if (
    ...
    def _validate_pool_data_logging(
        ...
        """
        self.mgr.logger.debug(f"[bot.py validate] pool_cid: {pool_cid}")
        self.mgr.logger.debug(
            f"[bot.py validate] fetched_pool: {fetched_pool['exchange_name']}"
        )
        self.mgr.logger.debug(f"[bot.py validate] fetched_pool: {fetched_pool}")
    ...
    def calculate_profit(
        ...
        """
        self.mgr.logger.debug(
            f"[bot.calculate_profit_usd] best_profit, fl_token, flashloan_fee_amt: {best_profit, fl_token, flashloan_fee_amt}"
        )
        sort_sequence = (
            ["bancor_v2", "bancor_v3"] + self.mgr.UNI_V2_FORKS + self.mgr.UNI_V3_FORKS
        )

        best_profit_fl_token = best_profit
        flashloan_fee_amt_fl_token = Decimal(str(flashloan_fee_amt))
        if fl_token not in [
            self.mgr.WRAPPED_GAS_TOKEN_ADDRESS,
            self.mgr.NATIVE_GAS_TOKEN_ADDRESS,
        ]:
            price_curves = self.get_prices_simple(
                CCm, self.mgr.WRAPPED_GAS_TOKEN_ADDRESS, fl_token
            )
            sorted_price_curves = self.custom_sort(price_curves, sort_sequence)
            self.mgr.logger.debug(
                f"[bot.calculate_profit sort_sequence] {sort_sequence}"
            )
            self.mgr.logger.debug(f"[bot.calculate_profit price_curves] {price_curves}")
            self.mgr.logger.debug(
            ...
            if len(sorted_price_curves) > 0:
                ...
                flashloan_fee_amt_gastkn = Decimal(
                    str(flashloan_fee_amt_fl_token)
                ) / Decimal(str(fltkn_gastkn_conversion_rate))
                best_profit_gastkn = (
                    Decimal(str(best_profit_fl_token))
                    / Decimal(str(fltkn_gastkn_conversion_rate))
                    - flashloan_fee_amt_gastkn
                )
                self.mgr.logger.debug(
                    f"[bot.calculate_profit] {fl_token, best_profit_fl_token, fltkn_gastkn_conversion_rate, best_profit_gastkn, 'GASTOKEN'}"
                )
            else:
                self.mgr.logger.error(
                    f"[bot.calculate_profit] Failed to get conversion rate for {fl_token} and {self.mgr.WRAPPED_GAS_TOKEN_ADDRESS}. Raise"
                )
                raise
        else:
        ...
        self.mgr.logger.debug(
            f"[bot.calculate_profit_usd] {'GASTOKEN', best_profit_gastkn, usd_gastkn_conversion_rate, best_profit_usd, 'USD'}"
        )
    ...
    def _handle_trade_instructions(
        self, CCm: CPCContainer, arb_mode: str, r: Any
    ) -> Any:
        """
        ...
        flashloan_fee = self.mgr.blockchain.flashloan_fee
        flashloan_fee_amt = flashloan_fee * (
            flashloan_amount_wei / 10 ** int(fl_token_decimals)
        )
        ...
        (
            best_profit_fl_token,
            best_profit_gastkn,
            best_profit_usd,
        ) = self.calculate_profit(CCm, best_profit, fl_token, flashloan_fee_amt)
        ...
        if best_profit_gastkn < self.mgr.DEFAULT_MIN_PROFIT_GAS_TOKEN:
            self.mgr.logger.info(
                f"[bot._handle_trade_instructions] Opportunity with profit: {num_format(best_profit_gastkn)} does not meet minimum profit: {self.mgr.DEFAULT_MIN_PROFIT_GAS_TOKEN}, discarding."
            )
            return None, None
        ...
        if self.mgr.ARB_CONTRACT_VERSION >= 10:
        ...
        if self.mgr.NETWORK == self.mgr.NETWORK_TENDERLY:
            return (
        ...
        tx_helpers = TxHelpers(ConfigObj=self.mgr)
    ...
    def _validate_and_submit_transaction_tenderly(
        ...
        return self.mgr.w3.eth.wait_for_transaction_receipt(tx)
        ...
        if CCm is None:
            ...
            if self.mgr.ARB_CONTRACT_VERSION < 10:
                filter_out_weth = [
                    ...
                    & (
                        (x.params.tkny_addr == self.mgr.WETH_ADDRESS)
                        or (x.params.tknx_addr == self.mgr.WETH_ADDRESS)
                    )
                ]
        ...
        while True:
            try:
                ...
                if tx_hash and tx_hash[0]:
                    self.mgr.logger.info(f"Arbitrage executed [hash={tx_hash}]")

                time.sleep(self.polling_interval)
            except self.NoArbAvailable as e:
                self.mgr.logger.debug(f"[bot:run:continuous] {e}")
            except Exception as e:
                self.mgr.logger.error(f"[bot:run:continuous] {e}")
                time.sleep(self.polling_interval)
    ...
    def run_single_mode(
        ...
        try:
            ...
            self.mgr.logger.warning(f"[NoArbAvailable] {e}")
        except Exception as e:
            self.mgr.logger.error(f"[bot:run:single] {e}")
            raise
    ...
    def _ensure_connection(self, tenderly_fork: str):
        ...
        self.mgr.w3 = Web3(Web3.HTTPProvider(tenderly_uri))
    ...
    def run(
        ...
        *,
        ...
        if self.TxSubmitHandler is None:
            self.TxSubmitHandler = TxSubmitHandler(ConfigObj=self.mgr)
        assert issubclass(
