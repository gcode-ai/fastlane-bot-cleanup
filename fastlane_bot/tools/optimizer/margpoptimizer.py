...
class MargPOptimizer(CPCArbOptimizer):
    ...
    def optimize(self, sfc=None, result=None, *, params=None):
        ...
        **at the moment only the trivial self-financing constraint is allowed, ie the one that
        ...
        """
        # staticdata conversion: string to SFC object; note that anything but pure arb not currently supported
        ...
        tokens_t = tuple(
            t for t in alltokens_s if t != targettkn
        )  # all _other_ tokens...
        ...
        try:
            ...
            if len(curves_t) == 1:
                raise self.ParameterError(
                    f"can't run arbitrage on single curve {curves_t}"
                )
            ...
            if not P("pstart") is None:
                ...
                if P("verbose") or P("debug"):
                    print(
                        f"[margp_optimizer] using pstartd [{len(P('pstart'))} tokens]"
                    )
                ...
                try:
                    ...
                    if P("verbose") or P("debug"):
                        print(
                            f"[margp_optimizer] error while calculating price estimates: [{e}]"
                        )
            ...
            if result == self.MO_PSTART:
                df = pd.DataFrame(
                    price_estimates_t, index=tokens_t, columns=[targettkn]
                )
                df.index.name = "tknb"
                return df
            ...
            def dtknfromp_f(p, *, islog10=True, asdct=False, quiet=False):
                ...
                for pair, (tknb, tknq) in zip(pairs, pairs_t):
                    if get(p, tokens_ix.get(tknq)) > 0:
                        price = get(p, tokens_ix.get(tknb)) / get(
                            p, tokens_ix.get(tknq)
                        )
                    else:
            ...
            if i >= maxiter - 1:
                raise self.ConvergenceError(
                    f"maximum number of iterations reached [{i}]"
                )
