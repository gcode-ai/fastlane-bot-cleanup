"""
Carbon helper module - retrieve staticdata from CryptoCompare
"""
__VERSION__ = "2.1"
...
class CryptoCompare:
    ...
    :datapath:      the path where all staticdata is written (and read from)
    ...
    APIKEYNAME = (
        "CCAPIKEY"  # the name of the environment variable containing the API key
    )
    RAISEONERROR = True
    ...
    def __init__(
        self, *, apikeyname=None, apikey=None, raiseonerror=None, verbose=False
    ):
        ...
        if apikey is None:
            ...
            if apikey is None:
                ...
                print(
                    f"Use `export {apikeyname}=<value>` to set it BEFORE you launch Jupyter"
                )
                raise RuntimeError(
                    f"API key not present. Use `export {apikeyname}=<value>` to set it before launching Jupyter."
                )
        self.apikey = apikey
    ...
    def datafn(self, fn):
        """returns the full staticdata file name, including path"""
        return _os.path.join(self.DATAPATH, fn)

    def cache(self, item):
        """
        reads a staticdata item from the staticdata cache
        """
        try:
    ...
    def write_cache(self, item, data):
        """
        writes `staticdata` to the cache under the name `item`
    ...
    def query_allexchanges(self):
        """
        endpoint = /staticdata/v4/all/exchanges

        https://min-api.cryptocompare.com/documentation?key=Other&cat=allExchangesV4Endpoint
        """
        r = self.query(endpoint="/staticdata/v4/all/exchanges")
        if r is None:
            return r
    ...
    def cache_allexchanges(self, readonfail=True, updateonfail=False):
        ...
        return self._cache_xxx(
            item="query_allexchanges", updatemethod=self.query_allexchanges
        )
    ...
    def query_ratelimit(self):
        ...
        """
        r = self.query(endpoint="/stats/rate/limit")
        if r is None:
            return r
        return r.json().get("Data")
    ...
    def cache_coinlist(self, readonfail=True, updateonfail=False):
        ...
        return self._cache_xxx(item="query_coinlist", updatemethod=self.query_coinlist)

    def query_indexlist(self):
        """
        endpoint = /staticdata/index/list

        https://min-api.cryptocompare.com/documentation?key=Index&cat=listOfIndices
        """
        r = self.query(endpoint="/staticdata/index/list")
        if r is None:
            return r
        return r.json().get("Data")
    ...
    def cache_indexlist(self, readonfail=True, updateonfail=False):
        ...
        return self._cache_xxx(
            item="query_indexlist", updatemethod=self.query_indexlist
        )
    ...
    def query_freqlypair(
        self, freq, fsym=None, tsym=None, e=None, limit=False, toTs=None, aspandas=True
    ):
        """
        endpoints = /staticdata/v2/histoday, /staticdata/v2/histohour, /staticdata/v2/histominute
        ...
        :e:        exchange to obtain staticdata from
        :limit:    number of staticdata points to return (max: 2000; False defaults to that number)
        :toTs:     returns historical staticdata BEFORE that timestamp
        ...
        if not freq in self.FREQS:
            raise ValueError(
                "Unknow frequency {}. Use the FREQ_XXX constants provided."
            )
        endpoint = f"/staticdata/v2/histo{freq}"
        params = {
            "fsym": fsym,
    ...
    def query_tokens(self, fsyms, tsym=None, aspandas=False):
        """
        endpoint = /staticdata/pricemulti?fsyms=BTC,ETH&tsyms=USD,EUR

        :fsyms:     list of cryptocurrency symbols of interest
        :tsym:      currency symbol to convert into
        :aspandas:  if True, returns result as pandas staticdata frame

        https://min-api.cryptocompare.com/documentation?key=Price&cat=multipleSymbolsPriceEndpoint
        """
        endpoint = f"/staticdata/pricemulti"
        params = {
            "fsyms": fsyms,
    ...
    def aggr_query(
        ...
        **kwargs,
        ...
        """
        gets the staticdata for pairs from the API and converts it into tables

        :pairs:             the pairs to download, either comma separeted "ETH/USD, BTC/GBP, ..."
                            or as tuple of tuples (("ETH", "USD"), ...)
        :fields:            the fields for which to create aggredate staticdata frames, either comma separated
                            or as tuple/list; use FREQ_CLOSE and other FIELD_XXX constants here
        :incl_raw:          whether to include the individual raw staticdata frames
        :incl_raw_aggr:     whether to include the aggregate raw staticdata frame
        :incl_grand_aggr:   whether to include a grand aggregate (with double col name)
        :freq:              the staticdata frequency [FREQ_DAILY (default), FREQ_HOURLY, FREQ_MINUTELY]
    ...
    def pairs_fields_from_df(df):
        ...
        :df:        staticdata frame with index = (base token, quote token, field)
    ...
    def reformat_raw_df(cls, df, field=None, dblcolnm=False):
        ...
        :returns:   the reformatted staticdata frame
        """
        if field is None:
