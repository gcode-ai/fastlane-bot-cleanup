...
class RouteStruct:
    ...
    _customData: dict
        The custom staticdata. Required for trades on Carbon. (unencoded)
    customData: bytes
        The custom staticdata abi-encoded. Required for trades on Carbon. (abi-encoded)
    """
...
class TxRouteHandler(TxRouteHandlerBase):
    ...
    def _abi_encode_data(
        ...
        """
        Gets the custom staticdata abi-encoded. Required for trades on Carbon. (abi-encoded)
    ...
    def handle_custom_data_extras(
        ...
        assert (
            custom_data in "0x"
        ), f"[routehandler.py handle_uni_v3_router_switch] Expected the custom staticdata field to contain '0x', but it contained {custom_data}. This function may need to be updated."
        if platform_id == self.ConfigObj.network.EXCHANGE_IDS.get(
            self.ConfigObj.network.UNISWAP_V3_NAME
        ):
            if (
    ...
    def _calc_carbon_output(
        ...
        """
        assert (
            tkn_in != self.ConfigObj.NATIVE_GAS_TOKEN_ADDRESS
        ), "[routehandler.py _calc_carbon_output] Function does not expect native gas token as input."
    ...
    def _calc_balancer_output(
        ...
        if amount_in > (tkn_in_balance * Decimal("0.3")):
            raise BalancerInputTooLargeError(
                "Balancer has a hard constraint that amount in must be less than 30% of the pool balance of tkn in, "
                "making this trade invalid."
            )
        ...
        if amount_out > (tkn_out_balance * Decimal("0.3")):
            raise BalancerOutputTooLargeError(
                "Balancer has a hard constraint that the amount out must be less than 30% of the pool balance of tkn "
                "out, making this trade invalid."
            )
