...
class TradeInstruction:
    ...
    def __post_init__(self):
        ...
        if self.tknout_dec_override is None:
            ...
            self.tknin_is_wrapped = (
                self.mgr.blockchain.wrapped_gas_token_address in tokens
            ...
            self.tknin_is_native = (
                self.mgr.blockchain.native_gas_token_address in tokens
                and self._tknin_address
                in [
                    self.mgr.blockchain.wrapped_gas_token_address,
                    self.mgr.blockchain.native_gas_token_address,
            ...
            assert (
                ...
                f"[TradeInstruction __post_init__] only 1 token can be native or wrapped, [self.tknin_is_wrapped, "
                f"self.tknin_is_native, self.tknout_is_wrapped, self.tknout_is_native"
                f"] = {self.tknin_is_wrapped, self.tknin_is_native, self.tknout_is_wrapped, self.tknout_is_native}"
            )
    ...
    def get_custom_int(self) -> int:
        ...
        if self.exchange_name in self.mgr.uniswap_v3_forks:
            ...
            custom_int = (
                0 if pool.pool_type != self.mgr.blockchain.POOL_TYPE_STABLE else 1
            )
    ...
    def get_platform_id(self):
        ...
        """
        return self.mgr.get_exchange_by_name(self._exchange_name).id
