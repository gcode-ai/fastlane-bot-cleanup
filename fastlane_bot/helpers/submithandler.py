...
from ..etl.models.abi import FAST_LANE_CONTRACT_ABI
...
class TxSubmitHandler(TxSubmitHandlerBase):
    ...
    def __post_init__(self):
        ...
        if self.mgr.blockchain.name in "ethereum":
            self.bancor_network_info = self.mgr.bancor_network_info_contract

        self.token_contract = self.mgr.w3.eth.contract(
    ...
    def _get_deadline(self) -> int:
        ...
        """
        return (
            self.mgr.w3.eth.get_block(self.mgr.w3.eth.block_number).timestamp
            + DEFAULT_BLOCKTIME_DEVIATION
        )
    ...
    def _get_transaction(self, tx_details: TxParams) -> TxParams:
        ...
        TxParams
            The transaction details.
        """
        return fill_nonce(self.mgr.w3, tx_details)
    ...
    def _get_transaction_receipt(self, tx_hash: str, timeout: int):
        ...
        """
        return self.mgr.w3.eth.wait_for_transaction_receipt(tx_hash, timeout=timeout)
    ...
    def _get_transaction_receipt_with_timeout(
        ...
        with Timeout(timeout) as _timeout:
            while True:
                tx_receipt = self.mgr.w3.eth.getTransactionReceipt(tx_hash)
    ...
    def _get_gas_price(self) -> int:
        ...
        """
        return int(self.mgr.w3.eth.gas_price * DEFAULT_GAS_PRICE_OFFSET)
    ...
    def _get_tx_details(self):
        ...
        return {
            ...
            "gas": DEFAULT_GAS,
            "from": self.mgr.w3.to_checksum_address(
                self.mgr.blockchain.fastlane_contract_address
            ),
            "nonce": self.mgr.w3.eth.get_transaction_count(
                self.mgr.w3.to_checksum_address(
                    self.mgr.blockchain.fastlane_contract_address
                )
            ),
        }
    ...
    def submit_transaction_tenderly(
        ...
        self.arb_contract = self.mgr.w3.eth.contract(
            address=self.mgr.w3.to_checksum_address(
                self.mgr.blockchain.fastlane_contract_address
            ),
            abi=FAST_LANE_CONTRACT_ABI,
        )
        print(
            f"Submitting transaction to Tenderly with endpoint_uri: {self.mgr.w3.provider.endpoint_uri}"
        )
        address = self.mgr.w3.to_checksum_address(BINANCE14_WALLET_ADDRESS)
    ...
    def _get_transaction_hash(self, tx_details: TxParams, key: str) -> str:
        ...
        str
            The transaction hash.
        """
        return self.mgr.w3.eth.send_raw_transaction(
            to_hex(
                self.mgr.w3.eth.account.sign_transaction(tx_details, key).rawTransaction
            )
        )
    ...
    def _get_nonce(self, address: str) -> int:
        ...
        """
        return self.mgr.w3.eth.get_transaction_count(address)
    ...
    def _build_transaction(
        ...
        return {
            **self._get_transaction_details(
            ...
            "staticdata": tx_function.buildTransaction(tx_params)["staticdata"],
        }
