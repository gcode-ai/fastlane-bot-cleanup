...
class CPCArbOptimizer(OptimizerBase):
    ...
    class SelfFinancingConstraints(DCBase):
        ...
        :staticdata:      a dict TKN -> amount, or AMMPays, AMMReceives
                    :amount:            from the AMM perspective, total inflows (>0) or outflows (<0)
                                        for all items not present in staticdata the value is assumed zero
        ...
        :tokens:    set of all tokens in the problem (if None, use staticdata.keys())
        ...
        def new(cls, tokens, **data):
            """alternative constructor: staticdata as kwargs"""
    ...
    def SFCd(self, data_dct):
        """alias for SelfFinancingConstraints.new, with staticdata as a dict"""
    ...
    class MargpOptimizerResult(OptimizerBase.OptimizerResult):
        ...
        def dxdyvalues(self, asdict=False):
            ...
            ), "cannot get this staticdata from an error result"
            result = (
