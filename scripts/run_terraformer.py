...
from fastlane_bot.constants import SOLIDLY_EXCHANGE_INFO
from fastlane_bot.etl.models.solidly.v2.exchange import SolidlyV2Exchange
from fastlane_bot.etl.utils import safe_int
...
def organize_pool_details_solidly_v2(
    ...
    pool_contract = async_web3.eth.contract(
        address=pool_address, abi=exchange_object.get_abi()
    )
    fee_float = asyncio.get_event_loop().run_until_complete(
        asyncio.gather(
            exchange_object.get_fee(address=pool_address, contract=pool_contract)
        )
    )
...
def _get_events(contract, blockchain: str, exchange: str, start_block: int) -> list:
    ...
    event_method = contract.events[
        EXCHANGE_POOL_CREATION_EVENT_NAMES[exchange]
    ].get_logs
    events_list = [
        event_method(fromBlock=block_numbers[n - 1], toBlock=block_numbers[n] - 1)
        for n in range(1, len(block_numbers))
    ]
...
def get_solidly_v2_pools(
    ...
    solidly_exchange = SolidlyV2Exchange(
        name=exchange, factory_contract=async_factory_contract
    )
...
def get_web3_for_network(network_name: str) -> Tuple[Web3, AsyncWeb3]:
    ...
    try:
        ...
        return None
    return Web3(Web3.HTTPProvider(f"{alchemy_rpc}{ALCHEMY_API_KEY}")), AsyncWeb3(
        AsyncWeb3.AsyncHTTPProvider(f"{alchemy_rpc}{ALCHEMY_API_KEY}")
    )
